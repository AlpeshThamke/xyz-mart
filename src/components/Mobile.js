import {Alert, Typography,Button,CssBaseline,AppBar,Box,Divider,Drawer,List,ListItem,ListItemIcon,ListItemText,Toolbar} from "@mui/material";
...
function Mobile(props){
    ...
    return(
        ...
        <Typography variant='h4' style={{fontFamily:"Poppins",fontSize:30,background:'red',color:"white",width:'40%',marginBottom:20,border:'1px solid red'}}>Top Mobile Brands at your doorstep</Typography>
        ...
        <Grid container justifyContent="center" spacing={4}>
            {MobileData.map((each) =>{
                ...
                return(
                    <Grid key={title} item>
                    <Paper sx={{height: 600,width: 400}} style={{background:"#f0f0f0",border:'1px solid black'}}>
                    <div>
                        <img style={{width:250,height:430,marginLeft:'18%'}} src={img} alt={title}/>
                        <Typography style={{fontFamily:"Poppins",fontSize:30,textAlign:"center"}}>{title}</Typography>
                        <Typography style={{fontFamily:"Poppins",fontSize:25,textAlign:"center"}}>&#8377; {price}</Typography>
                        <Button style={{textTransform:"none",fontFamily:'Poppins',fontSize:20,marginLeft:'26%'}} variant="contained" onClick={()=>setBought(true)} endIcon={<AddShoppingCartIcon/>}>Add to Cart</Button>
                    </div>
                </Paper>
                </Grid>
                )
